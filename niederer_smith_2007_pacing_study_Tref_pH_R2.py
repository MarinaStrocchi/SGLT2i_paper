...
def createLegends():
    ...
    legend_constants[197] = "scale factor for j_r for sensitivity analsis"
    legend_constants[198] = "scale factor for NCX for sensitivity analsis"
    legend_constants[199] = "scale factor for SERCA for sensitivity analsis"
...
def initConsts(args):
    ...
    constants[16] = -10
    constants[17] = 1.2e-6 * float(args.Ko_scale_factor)
    constants[18] = 0.0008 * float(args.Na_scale_factor)
    ...
    constants[20] = 3.5e-5 * float(args.Kt_scale_factor)
    constants[21] = 0.583
    constants[22] = 0.417
    constants[23] = 5.4
    ...
    constants[24] = 7e-6 * float(args.Kss_scale_factor)
    ...
    constants[25] = 2.4e-5 * float(args.K1_scale_factor)
    constants[26] = 1.45e-6 * float(args.Kf_scale_factor)
    constants[27] = 0.2 * float(args.Naf_scale_factor)
    ...
    constants[32] = 9.5e-5 * float(args.NaK_scale_factor)
    constants[33] = 1.5
    ...
    constants[49] = 257.4
    constants[50] = 4084.9
    constants[51] = 0.35 * float(args.che_scale_factor)
    ...
    constants[60] = 2.18
    constants[61] = 0.35 * float(args.ncb_scale_factor)
    constants[62] = 983.5
    constants[63] = 110.64
    ...
    constants[70] = 1.44
    constants[71] = 0.35 * float(args.ae_scale_factor)
    constants[72] = 0.05
    ...
    constants[81] = 1
    constants[82] = 2e-5 * float(args.Cl_scale_factor)
    constants[83] = 4e-6 * float(args.H_scale_factor)
    constants[84] = 0
    constants[85] = 2
    constants[86] = 3.5e-6 * float(args.pCa_scale_factor)
    constants[87] = 0.0005
    states[16] = 8.60587059887361e-5 if not initialise else initial_state[16]
    constants[88] = 0.075
    constants[89] = 2.4216e-8 * float(args.Cab_scale_factor)
    constants[90] = 87.5
    constants[91] = 1.38
    constants[92] = 0.35
    constants[93] = 0.1
    ...
    constants[99] = 5.2e-6 * float(args.leak_scale_factor)
    constants[100] = 0.07
    constants[101] = 6.5e-11
    constants[102] = 2e-11
    constants[103] = 9.13e-13 * float(args.CaL_scale_factor)
    constants[104] = -2
    constants[105] = 7
    constants[106] = 1
    ...
    # added to change them in computeRates and computeAlgebraic
    constants[197] = float(args.jRel_scale_factor)
    constants[198] = float(args.ncx_scale_factor)
    constants[199] = float(args.serca_scale_factor)
...
def computeRates(voi, states, constants):
    ...
    # SERCA
    constants[95] = 0.00045 * constants[199] * (1.0 + numpy.power(10.0000, constants[190] * (-constants[189] + constants[191])))/(1.0 + numpy.power(10.0000, constants[190] * (-states[13] + constants[191])))
 
    # NCX
    constants[94] = 0.0385 * constants[198] * (1.0 + numpy.power(10.0000, constants[192] * (-constants[189] + constants[193])))/(1.0 + numpy.power(10.0000, constants[192] * (-states[13] + constants[193])))

    # RyR
    constants[102] = 2e-11 * constants[197] * (1.0 + numpy.power(10.0000, constants[194] * (-constants[189] + constants[195])))/(1.0 + numpy.power(10.0000, constants[194] * (-states[13] + constants[195])))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--iK_b_scale_factor', type=str, default="1.0", required=False,
