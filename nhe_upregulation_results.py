...
source_folder = "./results/nhe_upregulation/rat_model/"
fig_folder = source_folder
if not os.path.exists(fig_folder):
    os.system("mkdir -p "+fig_folder)

metrics_data = pd.read_csv(os.path.join(source_folder,"rat_model_nhe_upregulation_with_baseline.csv"),index_col=0)
...
colours = ["deepskyblue","tab:red","tab:red"]
hatches = ['','','///']
line_styles = ['-','-','--']
dashes = [[],[],(5, 1)]

sims_to_compare = ["HF_pH","HF_NHE3","HF_pH_NHE_off"]
legend = ["$[Na^+]_i$"" ++","$[Na^+]_i$"" \& NHE ++","$[Na^+]_i$ ++\nNHE off"]
...
plt.tight_layout()
plt.savefig(os.path.join(fig_folder,"nhe_upregulation.png"),dpi=300)
